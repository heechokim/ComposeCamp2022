...
- 컴포즈는 컴포저블 함수를 정의함으로써 UI 요소를 만들고 컴포저블 함수들을 호출해서 화면의 UI를 구성합니다. 컴포저블 함수는 굉장히 단순한 함수입니다. 컴포저블 함수를 정의할 때 `@Composable` 어노테이션을 붙여주고 함수의 파라미터로 UI 요소에 연결해줄 데이터를 전달해주면 끝입니다. @Composable 어노테이션을 붙여주어야 컴포즈 컴파일러가 '아 이 함수는 컴포저블 함수군' 라고 인식할 수 있기 때문입니다. 
- ```kotlin
  // 컴포저블 함수 호출 예시
