...
- 따라서 컴포저블 함수 내에서 변수를 선언하고 변수의 값을 바꾸는 로직은 정상 동작을 보장할 수 없습니다.
  ...
  ~~~
  Column() 컴포저블 함수(주석1)와 Text() 컴포저블 함수(주석2)는 호출 순서가 정해져있지 않으므로, Column 내부의 for문 안의 코드가 동작하는 동안 Text() 컴포저블 함수(주석2)가 먼저 호출될 수 있습니다. 이 상황에서 Text() 컴포저블 함수(주석2)가 호출되는 시점의 itemCount 값은 의도한 값이 아닐 수 있습니다.
